...
The animation isn't pulled from a library. An AI model writes [Manim](https://www.manim.community/) code (the animation engine behind 3Blue1Brown's videos) for that exact problem, and Clarity renders it.
...
| **Agent service** | The only part that talks to the AI model (Claude). Reads the screenshot, writes the explanation, writes the Manim code, fixes it when it breaks | Python | P1 |
...
Supporting services: **MongoDB Atlas** (stores jobs, the cache, and a library of verified Manim examples the AI learns from), **S3 / MinIO** (stores finished videos), **Claude** (the model), **Voyage AI** (turns text into vectors so we can search the example library by meaning).
...
| **Agent service** | The Python server that makes every call to Claude. |
