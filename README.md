...
Supporting services: **MongoDB Atlas** (stores jobs, the cache, and a library of verified Manim examples the AI learns from), **S3 / MinIO** (stores finished videos), **Google Gemini** (reads the screenshot), **Claude** (explanation and code), **Voyage AI** (turns text into vectors so we can search the example library by meaning).
...
| **Agent service** | The Python server that makes every AI call — Gemini to read the screenshot, Claude to explain and to write code. |
